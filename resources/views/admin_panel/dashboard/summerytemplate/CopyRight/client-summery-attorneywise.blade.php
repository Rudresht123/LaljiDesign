        ...
        <div class="custom-card">
            <div class="panel  m-0 p-0  panel-default">
                <div class="panel-heading border-bottom mb-2">
                    <h6 class="tx-14 m-0 p-0"><b class="d-flex"><i class="far fa-address-book me-1"></i>Trademark Attorneys Wise
                                    ...
                                    <div class="row d-felx">
                                        <div class=" col-12 pl-4 d-flex justify-content-between">
                                            ...
                                            <span class="badge text-bg-danger"
                                                style="font-size: 8px;">{{ isset($copyrightdatacount[$attoerny->id]) && $copyrightdatacount[$attoerny->id] ? $copyrightdatacount[$attoerny->id] : 0 }}</span>
                                        </div>
                                        ...
                                        <div class="col-lg-12 m-0 p-0">
                                            <table cellspacing="0" cellpadding="0"
                                                ...
                                                <tbody>
                                                    ...
                                                    @foreach ($copyrightgroupedData as $copygroupdata)
                                                    @if ($attoerny->id === $copygroupdata->attorney_id)
                                                    @php
                                                    ++$counter;
                                                    @endphp
                                                    <tr class="fs-10">
                                                        <td>
                                                            <a
                                                                href="{{ route('admin.attorney.clientDetails', ['category_slug' => $copygroupdata->mainCategory->category_slug, 'id' => $copygroupdata->id]) }}">
                                                                {{ $copygroupdata->application_no ?? '' }}
                                                            </a>
                                                        </td>
                                                        <!-- Placeholder for profile image -->
                                                        <td>{{ $copygroupdata->file_name ?? '' }}</td>
                                                        <td style="word-wrap:nowrap;">
                                                            {{ $copygroupdata->trademark_name ?? '' }}
                                                        </td>
                                                        <td>{{ $copygroupdata->phone_no ?? '' }}</td>
                                                        <td>{{ $copygroupdata->statusMain->status_name ?? '' }}
                                                        </td>
                                                    </tr>
                                                    @endif
                                                    @endforeach
