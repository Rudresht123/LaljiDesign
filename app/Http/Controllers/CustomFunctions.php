...
class CustomFunctions extends Controller
{
    ...
    public function searchCleint(Request $request)
    {
        ...
        $attoernyes = $userPermissionRepository->getAtteorneys();

        if ($request->filled('inputText')) {
            $searchValue = trim($request->input('inputText'));

            // Search in Trademark
            $trademarkResults = TrademarkUserModel::where(function ($q) use ($searchValue) {
                $q->where('application_no', 'like', '%' . $searchValue . '%')
                    ->orWhere('file_name', 'like', '%' . $searchValue . '%')
                    ->orWhere('trademark_name', 'like', '%' . $searchValue . '%');
            })
                ->whereIn('attorney_id', $attoernyes)
            ...
            // Search in Copyright
            $copyrightResults = CopyRightUserModel::where(function ($q) use ($searchValue) {
                $q->where('application_no', 'like', '%' . $searchValue . '%')
                    ->orWhere('file_name', 'like', '%' . $searchValue . '%')
                    ->orWhere('trademark_name', 'like', '%' . $searchValue . '%'); // rename field if needed
            })
                ->whereIn('attorney_id', $attoernyes)
                ->select('id', 'application_no', 'file_name', 'category_id')
                ->get()
            ...
            // Merge all results
            $mergedResults = $trademarkResults
                ->merge($copyrightResults);

            if ($mergedResults->isEmpty()) {
                return response()->json(['message' => 'No records found for this search.']);
            }

            return response()->json($mergedResults->values());
        }
    ...
    {
        $categoryname = MainCategoryModel::where('id', $request->category_id)->pluck('category_slug')->first();
    ...
    public function getClients(Request $request)
    {
        ...
        $categories = MainCategoryModel::where('id', $request->maincategory)->get();
        ...
        foreach ($categories as $category) {
            switch ($category->category_slug) {
                ...
                case 'copyright':
                    $tableColumns = Schema::getColumnListing('copyright_users');
                    $query = CopyRightUserModel::with([
                        'attorney:id,attorneys_name',
                        'mainCategory:id,category_name,category_slug',
                        'office:id,office_name',
                        'statusMain:id,status_name',
            ...
            if (Auth::user()->role === 'superadmin') {
                ...
                $userPermissionRepository = new UserPermissionRepo();
                $attorneysId = $userPermissionRepository->getAtteorneys();
    
                if ($request->filled('attorney_id')) {
                    $query->whereIn('attorney_id', $request->attorney_id);
                } else {
                    $query->whereIn('attorney_id', $attorneysId);
                }
            }
            ...
            if ($request->filled('start_date') && $request->filled('from')) {
                $startDate = \Carbon\Carbon::parse($request->start_date)->startOfDay();
                $endDate = \Carbon\Carbon::parse($request->from)->endOfDay();
                $query->whereBetween('created_at', [$startDate, $endDate]);
            }
    
            if ($request->filled('start_date') && $request->filled('end_date')) {
                $query->whereBetween('filling_date', [$request->start_date, $request->end_date]);
            }
    
            $queries[] = $query;
        }
    
        // Combine multiple queries using union (if needed)
        if (count($queries) === 0) {
            return response()->json([
                'draw' => intval($request->input('draw')),
                'recordsTotal' => 0,
        ...
        /** @var \Illuminate\Database\Eloquent\Builder $finalQuery */
        ...
        if ($request->input('search.value')) {
            ...
            $finalQuery->where(function ($q) use ($searchValue) {
                $q->where('application_no', 'like', '%' . $searchValue . '%')
                  ->orWhere('trademark_name', 'like', '%' . $searchValue . '%')
                  ->orWhere('file_name', 'like', '%' . $searchValue . '%');
            });
        ...
        if ($request->has('order')) {
            ...
            if (isset($columns[$columnIndex])) {
                $columnName = $columns[$columnIndex]['name'];
                $finalQuery->orderBy($columnName, $sortDirection);
            }
        } else {
            $finalQuery->orderBy('id', 'asc');
        }
        ...
        $filteredRecords = $finalQuery->count();
        $data = $finalQuery->skip($start)->take($length)->get();
    
        // Total Records
        $totalRecords = TrademarkUserModel::count() + CopyRightUserModel::count();
        ...
        $formattedData = $data->transform(function ($item, $index) use ($start, $tableColumns) {
            $rowData = ['DT_RowIndex' => $start + $index + 1];
    
            foreach ($tableColumns as $tableColumn) {
                $rowData[$tableColumn] = ($tableColumn == 'application_no')
                    ? '<a href="' . route('admin.attorney.clientDetails', [
                        'category_slug' => $item->mainCategory->category_slug,
                        'id' => $item->id
                    ]) . '">' . $item->$tableColumn . '</a>'
                    : $item->$tableColumn;
            ...
            if (auth()->user()->hasPermission('admin.client-details.print-pdf')) {
                $rowData['actions'] .= '
                    <a target="_blank" class="dropdown-item" href="' . route('admin.client-details.print-pdf', [
                        'category_slug' => $item->mainCategory->category_slug,
                        'id' => $item->id
            ...
            if (auth()->user()->hasPermission('admin.attorney.edit-clientDetails')) {
                $rowData['actions'] .= '
            ...
            if (auth()->user()->hasPermission('admin.status.client-status')) {
                $rowData['actions'] .= '
                    <a target="_blank" class="dropdown-item" href="' . route('admin.status.client-status', [
                        'id' => $item->id
                    ]) . '">
            ...
            if (auth()->user()->hasPermission('admin.attorney.clientDetails')) {
                $rowData['actions'] .= '
                    <a class="dropdown-item" href="' . route('admin.attorney.clientDetails', [
                        'category_slug' => $item->mainCategory->category_slug,
                        'id' => $item->id
