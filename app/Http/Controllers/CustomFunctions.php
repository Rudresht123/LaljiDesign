...
class CustomFunctions extends Controller
{
      ...
      public function getClients(Request $request)
    {
      ...
      if(Auth::user()->role=='superadmin'){
        ...
        if ($request->filled('attorney_id')!=null) {
            $query->whereIn('attorney_id', $request->attorney_id);
        }
        else{
    ...
    // Get the start of the day (00:00:00) and end of the day (23:59:59)
    $startDate = \Carbon\Carbon::parse($request->start_date)->startOfDay();  // 00:00:00
    $endDate = \Carbon\Carbon::parse($request->from)->endOfDay();  // 23:59:59
...
// Check and add the "PDF Print" link if the user has permission
...
$rowData['actions'] .= '
    ...
    public function searchCleint(Request $request) {
        ...
        if ($request->input('inputText')) {
            ...
            $results = $query->whereIn('attorney_id', $attoernyes)->get();
